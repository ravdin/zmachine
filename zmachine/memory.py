...
class MemoryMap:
    def __init__(self, config: ZMachineConfig, graphics_enabled: bool = False):
        with open(config.game_file, 'rb') as f:
            data = f.read()
        self._memory_map = bytearray(data)
        self._version = self._memory_map[0]
        self._static_memory_base_addr = config.static_memory_base_addr
        self._graphics_enabled = graphics_enabled
        if self._version <= 3:
            # Split screen available.
            self._flags1_mask = 0x20
        if self._version >= 4:
            # Bold, italic, and fixed width font available.
            # Timed input available.
            self._flags1_mask = 0x9c
        if self._version == 5:
            # Colors available.
            self._flags1_mask |= 0x1
    ...
    def write_byte(self, addr: int, val: int):
        ...
        if addr >= self._static_memory_base_addr:
    ...
    def write_word(self, addr: int, val: int):
        ...
        if addr + 1 >= self._static_memory_base_addr:
            raise IllegalWriteException(addr)
    ...
    def reset_dynamic_memory(self, dynamic_mem: bytes):
        ...
        # Preserve bits 0 and 1 (transcript and fixed pitch) for flags2.
        # Preserve the height/width settings, which the game may or may not honor.
        flags2_mask = self.read_word(0x10) & 0x3
        ...
        self._memory_map[:self._static_memory_base_addr] = dynamic_mem
        self.set_screen_flags()
        flags2 = self.read_word(0x10) | flags2_mask
