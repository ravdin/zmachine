...
class Quetzal:
    def __init__(self, memory_map: MemoryMap, terminal: TerminalAdapter):
    ...
    def write_to_screen(self, text, newline=False):
        ...
        self.terminal.write_to_screen(text)
        if newline:
            self.terminal.write_to_screen('\n')

    def interpreter_prompt(self, prompt: str, lowercase: bool = False) -> str:
        """Prompt the user for input and return the result."""
        return self.terminal.get_input_string(prompt, lowercase)
