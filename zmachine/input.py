...
class InputStreamManager:
    ...
    def get_selected_stream_type(self) -> InputStreamType:
        if self.active_stream == self.keyboard_input_stream:
            return InputStreamType.KEYBOARD
        elif self.active_stream == self.playback_input_stream:
            return InputStreamType.PLAYBACK
        else:
            raise Exception("Unrecognized input stream selected")
            ...
            if c in TerminalEscape.values():
                ...
                if c in Hotkey:
                    ...
                    if c == Hotkey.HELP:
                        ...
                        selected_stream_type = self.input_source.get_selected_stream_type()
                        if selected_stream_type == InputStreamType.KEYBOARD:
                            self.hotkey_handler.open_record_stream()
                        elif selected_stream_type == InputStreamType.PLAYBACK:
                            self.hotkey_handler.close_record_stream()
                    elif c == Hotkey.DEBUG:
