...
class HotkeyHandler:
    ...
    def toggle_record_stream(self):
        if self.output_stream_manager.record_stream.is_active:
    ...
    def open_record_stream(self):
        ...
        self.output_stream_manager.record_stream.open(record_file_path)

    def close_record_stream(self):
        self.output_stream_manager.record_stream.close()
            ...
            for line in playback_file:
                if in_metadata_section:
                    if line.startswith('# SEED:'):
                        ...
                        self.terminal_adapter.write_to_screen("Invalid playback file format.\n")
                        return False
                else:
                    if line == '\n':
                        commands += [line]
